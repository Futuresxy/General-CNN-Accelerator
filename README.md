...
Accelerator IP硬件配置参数
|  硬件IP参数名称 | 说明                | 备注                                                         |
|  ---------- | ------------------- | ------------------------------------------------------------ |
|          DATA_WIDTH |  单个输入数据支持位宽      | 硬件数据位宽|
|          PARA_WIDTH |  单个控制寄存器参数位宽     | 硬件参数位宽|
|          RESULT_NUM  |  最大支持单次并行输出计算结果个数|  —— |
|          RESULT_DATA_WIDTH| 输出结果位宽 |    —— |
|          CONV_KERNEL_SIZE | 最大支持卷积阵列kernel大小 |单边长，此处要大于等于上面寄存器的数字动态配置范围|
|          CONV_KERNEL_NUM  | 最大支持卷积核个数 | 硬件卷积核个数，此处要大于等于上面寄存器的数字动态配置范围|
|          POOL_KERNEL_SIZE | 最大支持池化阵列kernel大小|池化核大小，此处要大于等于上面寄存器的数字动态配置范围|
|          POOL_KERNEL_NUM  |  最大支持池化核个数|仅支持1 |
|          FULLCON_INPUT_SIZE|最大全连接层输入大小 |此处要大于等于上面寄存器的对应数字动态配置范围|
|          FULLCON_OUTPUT_SIZE|全连接层输出大小 |此处要大于等于上面寄存器的数字动态配置范围|
#### 2.数据接口
...
## 4.对比测试
在硬件平台预设卷积脉动阵列最大支持 5×5 卷积计算，输入数据最大支持 40×5 滑动窗数据缓存且不填充数据，最大支持 8 个卷积算子同时计算的硬件设置下，测试加速器卷积计算对比 CPU 计算的加速效果。如表 4.2所示，对于在不同卷积输入数据、卷积核尺寸、步进、卷积核数量设置下进行实验对比，发现随着输入激励数据增大，加速器并行化计算的加速效果越明显。在输入数据为 40×40，卷积核尺寸为 5×5，卷积核数量为 8 时，对比CPU 计算达到了近 170 倍加速效果。在数据量较小、卷积核数量较少时也能达到十倍以上的加速效果。
![image](https://github.com/user-attachments/assets/9c02efb4-af8a-4d50-932c-452b31229a7a)

在 ReLU 模块加速性能测试方面，结合卷积计算进行组合模块加速测试，因为在 CPU端实现近似 ReLU 计算需要大量的判断，因此设计了如表 4.4所示的测试实验，选取了两种输入数据尺寸，改变不同的卷积核心进行实验测试，发现在卷积同样参数设置下 ReLU 模块的加入使得组合计算对比 CPU 又有了一定的提升，尤其是在数据量计算增大时，在输入为 28×28，卷积核尺寸为 5×5，数量为 4 条件下，对比 CPU 实现了约 260 倍的加速效果。
![image](https://github.com/user-attachments/assets/fb04fce1-7807-44e8-9bcf-4022fd50ccda)

在池化计算加速测试中，设置了最大池化、平均池化的测试，如表 4.5所示选取了两种输入激励尺寸和三种池化计算核心尺寸进行测试。发现在数据量较小时加速器的池化计算速度与 CPU 相近，在数据量较大时对比 CPU 计算，最大池化可以达到约 15 倍的提升，因为平均池化存在一个取平均操作，因此计算时间明显大于最大池化操作，加速器的速度提升也更为明显，对比 CPU 最大加速效果可达到 23 倍。
![image](https://github.com/user-attachments/assets/0df1d624-3d70-46a5-b9e3-180e8b584639)

全连接模块的加速性能测试选取了三种输入尺寸和三种输出尺寸进行测试，测试结果如表 4.6所示。因为全连接模块的脉动阵列横向尺寸为 1，所以加速效果不如卷积加速效果明显。在输入数据量较小时与 CPU 计算速度相近，在输入尺寸长度不超过 100 时，随着数据计算量加大可以达到近 7 倍的速度提升。
![image](https://github.com/user-attachments/assets/27077749-755b-44d2-a03c-4355910c88f1)

在 CPU 端侧也进行相同的网络计算设计，对比结果如表 4.7 所示，在单个网络结构层运行测试中，第一层的卷积融合 ReLU 加速效果最好，因为调用一次加速器卷积计算即可完成两个网络，对比 CPU 运算可提速 267.9 倍。不同于第二个卷积融合 ReLU, 单个卷积计算数据降采样，并且多通道的数据造成多次调用加速器卷积操作和 CPU 数据和运算时间，因此加速效果明显降低。最大池化层在输入数据量较大时有 24.1 倍的加速，但是因为并行度为 1，在数据降采样和通道增加时，加速效果减小。全连接层硬件乘加计算加速对比CPU 可达到加速接近 40 倍的效果。同时 CNN 加速器中的数据分离和搬运也占用了较多的时间。在整体网络完整运行测试中，CNN 加速器对比纯 CPU 计算加速比可达 14.30 倍，有明显的计算提速。
![image](https://github.com/user-attachments/assets/d2a557a7-7da1-47c8-9c79-baaf2e674342)
